...
object.list <- list(MHC = cellchat_MHC,CTR = cellchat_CTR)
...
ggsave(filename=paste0("MHCvsCTRNumber_of_Interaction_Comparison.png"), plot= arrange, width = 10, height = 8, units = 'in', dpi = 300)
...
png("MHCvsCTR_Network.png", width = 960, height = 960, units = "px")
print(netVisual_diffInteraction(cellchat, weight.scale = T))
dev.off()

png("MHCvsCTR_Network_weight.png", width = 960, height = 960, units = "px")
print(netVisual_diffInteraction(cellchat, weight.scale = T, measure = "weight"))
dev.off()

#PDFs of same plots
pdf("MHCvsCTR_Network.pdf")
print(netVisual_diffInteraction(cellchat, weight.scale = T))
dev.off()

pdf("MHCvsCTR_Network_weight1122.pdf")
print(netVisual_diffInteraction(cellchat, weight.scale = T, measure = "weight"))
dev.off()
...
pdf("MHCvsCTR_VisualHeatmap.pdf")
par(mfrow = c(1,2), xpd=TRUE)

print(netVisual_heatmap(cellchat))
dev.off()

pdf("MHCvsCTR_VisualHeatmap_weight.pdf")
...
for (i in 1:length(object.list)) {
  png(paste0("MHCvsCTR_Interactions,",i,".png"), width = 960, height = 960, units = "px") 
...
pdf("MHCvsCTR_DiffinterationNumberandStrength.pdf")
par(mfrow = c(1,2))#, xpd=TRUE)
...
pdf("MHCvsCTR_MajorSourcesandTargesIn2DSpace.pdf")
...
for (n in 1:25){
pdf(paste0("MHCvsCTR_SignalingChanges_Scatterplot_C",n,".pdf"))
...
ggsave(gg1, device = "pdf", filename = "Pathwaysimilarities_CTR_MHC.pdf")
...
#> Classification learning of the signaling networks for datasets 1 2
# Visualization in 2D-space
#pdf("MHCvsCTR_Signalinggroups_basedon_structural similatries.pdf")
gg1 <- netVisual_embeddingPairwise(cellchat, type = "structural", label.size = 8, top.label = T)
ggsave(gg1, device = "pdf", filename = "MHCvsCTR_Signalinggroups_basedon_structural similatries.pdf")
#dev.off()
#> 2D visualization of signaling networks from datasets 1 2
#pdf("MHCvsCTR_Signalinggroups_basedon_structural similatries_Pairwisezoomin.pdf")
gg1 <- netVisual_embeddingPairwiseZoomIn(cellchat, type = "structural", nCol = 2,
                                         show.legend = T)
ggsave(gg1, device = "pdf", filename = "MHCvsCTR_Signalinggroups_basedon_structural similatries_Pairwisezoomin.pdf")
#> 2D visualization of signaling networks from datasets 1 2
#dev.off()
...
#pdf("MHCvsCTR_Difference_in_Pathwaydistance_higher_Score more different between both groups.pdf")
gg1 <- rankSimilarity(cellchat, type = "functional")
ggsave(gg1, device = "pdf", filename = "MHCvsCTR_Difference_in_Pathwaydistance_higher_Score more different between both groups.pdf")
#dev.off()

rankSimilarity2df(cellchat, type = "functional")

write.csv(distance, file="MHCvsCTR_distance.csv")
...
#pdf("MHCvsCTR_Informationflow_of_each_pathways_comparison.pdf")
gg <- grid.arrange(gg1 + gg2, nrow = 1)
ggsave(gg, device = "pdf", filename = "MHCvsCTR_Informationflow_of_each_pathways_comparison.pdf")
...
rank_table <- rank_table %>% group_by(group) %>% mutate( diff = filter(rank_table,group == "MHC")$contribution.scaled  - filter(rank_table, group == "CTR")$contribution.scaled)

rank_table <-rank_table %>% mutate(namecolor = ifelse(rank_table$pvalues<0.05 && rank_table$group =="MHC", "MHC",ifelse(rank_table$pvalues<0.05 && rank_table$group =="CTR","CTR","NOT")))

write.csv(rank_table, file= "MHCvsCTR_rank_table_org.csv")
...
labs(title="Comparison of activated pathways between both HCM mutants",
      x ="<- activated in MyHC          activated in MHC ->   ", y = "pathways")+ 
...
ggsave(gg1, device = "pdf", filename = "MHCvsCTR_Comparison_BubblePlot_pdf", width = 14, height = 10, units = 'in', dpi = 300 )
...
save.image("Cellchat_woweights_MHCvsCTR.Rdata")
...
pdf("MHCvsCTR_Outgoing_Signalpattern_Comparison_Heatmap.pdf")#, paper = "a3")
draw(ht1 + ht2, ht_gap = unit(0.2, "cm"))
#gg1 <- grid.arrange(ht1+ht2)
#ggsave(gg1, device = "pdf", filename = "MHCvsCTR_Outgoing_Signalpattern_Comparison_Heatmap.pdf")
...
pdf("MHCvsCTR_Incoming_Signalpattern_Comparison_Heatmap.pdf")
...
pdf("MHCvsCTR_Overall_Signalpattern_Comparison_Heatmap.pdf")
draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))
dev.off()
...
{
pdf(paste0("MHCvsCTR_ligand-receptor_pairs_Cluster_",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
...
#> Comparing communications on a merged object
pdf(paste0("MHCvsCTR_Increased_Decreased_Signaling_Comparison_by_communication probabities",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
gg1 + gg2
dev.off()
...
net.down <- subsetCommunication(cellchat, net = net, datasets = "MHC",ligand.logFC = -0.1, receptor.logFC = -0.1)
...
#> Comparing communications on a merged object
pdf(paste0("MHCvsCTR_Up-Downregulated_Signaling_Comparison_by_differentialGeneExpression",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
gg1 + gg2
dev.off()
...
# Chord diagram
par(mfrow = c(1,2), xpd=TRUE)
pdf(paste0("MHCvsCTR_Up-Downregulated_Signaling_Comparison_by_differentialGeneExpression_Chordplot",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
...
pdf(paste0("MHCvsCTR_Up-Interactionpathways",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
#
for (i in 1:length(object.list)) {
...
pathways <- intersect(object.list[["MHC"]]@netP[["pathways"]],object.list[["CTR"]]@netP[["pathways"]])
...
pdf(paste0("MHC_vs_CTR_Circleplot_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
#> 
#> Do heatmap based on a single object
pdf(paste0("MHC_vs_CTR_Heatmapt_",x,"_Pathway.pdf"), paper = "a4r")
...
# Chord diagram
pdf(paste0("MHC_vs_CTR_Chordplot_",x,"_Pathway.pdf"), paper = "a4r")
...
pdf(paste0("MHC_vs_CTR_Chordplot_grouped_Celltypes_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
cellchat@meta$datasets = factor(cellchat@meta$datasets, levels = c("MHC", "CTR")) # set factor level
...
ggsave(ggq, device = "pdf", filename = paste0("MHCvsCTR_Genexpression_VlnPlot_Singnalingpathway",x,".pdf"))
}

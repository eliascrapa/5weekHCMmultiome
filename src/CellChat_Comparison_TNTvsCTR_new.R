...
data.dir <- './comparison_TNTvsCTR_woWeights'
dir.create(data.dir)
setwd(data.dir)
...
object.list <- list(TNT = cellchat_TNT,CTR = cellchat_CTR)
...
ggsave(filename=paste0("TNTvsCTRNumber_of_Interaction_Comparison.png"), plot= arrange, width = 10, height = 8, units = 'in', dpi = 300)
...
png("TNTvsCTR_Network.png", width = 960, height = 960, units = "px")
print(netVisual_diffInteraction(cellchat, weight.scale = T))
dev.off()

png("TNTvsCTR_Network_weight.png", width = 960, height = 960, units = "px")
print(netVisual_diffInteraction(cellchat, weight.scale = T, measure = "weight"))
dev.off()

#PDFs of same plots
pdf("TNTvsCTR_Network.pdf")
print(netVisual_diffInteraction(cellchat, weight.scale = T))
dev.off()

pdf("TNTvsCTR_Network_weight1122.pdf")
print(netVisual_diffInteraction(cellchat, weight.scale = T, measure = "weight"))
dev.off()
...
pdf("TNTvsCTR_VisualHeatmap.pdf")
par(mfrow = c(1,2), xpd=TRUE)

print(netVisual_heatmap(cellchat))
dev.off()

pdf("TNTvsCTR_VisualHeatmap_weight.pdf")
...
for (i in 1:length(object.list)) {
  png(paste0("TNTvsCTR_Interactions,",i,".png"), width = 960, height = 960, units = "px") 
...
pdf("TNTvsCTR_DiffinterationNumberandStrength.pdf")
par(mfrow = c(1,2))#, xpd=TRUE)
...
pdf("TNTvsCTR_MajorSourcesandTargesIn2DSpace.pdf")
...
for (n in 1:25){
pdf(paste0("TNTvsCTR_SignalingChanges_Scatterplot_C",n,".pdf"))
...
ggsave(gg1, device = "pdf", filename = "Pathwaysimilarities_CTR_TNT.pdf")
...
#> Classification learning of the signaling networks for datasets 1 2
# Visualization in 2D-space
#pdf("TNTvsCTR_Signalinggroups_basedon_structural similatries.pdf")
gg1 <- netVisual_embeddingPairwise(cellchat, type = "structural", label.size = 8, top.label = T)
ggsave(gg1, device = "pdf", filename = "TNTvsCTR_Signalinggroups_basedon_structural similatries.pdf")
#dev.off()
#> 2D visualization of signaling networks from datasets 1 2
#pdf("TNTvsCTR_Signalinggroups_basedon_structural similatries_Pairwisezoomin.pdf")
gg1 <- netVisual_embeddingPairwiseZoomIn(cellchat, type = "structural", nCol = 2,
                                         show.legend = T)
ggsave(gg1, device = "pdf", filename = "TNTvsCTR_Signalinggroups_basedon_structural similatries_Pairwisezoomin.pdf")
#> 2D visualization of signaling networks from datasets 1 2
#dev.off()
...
#pdf("TNTvsCTR_Difference_in_Pathwaydistance_higher_Score more different between both groups.pdf")
gg1 <- rankSimilarity(cellchat, type = "functional")
ggsave(gg1, device = "pdf", filename = "TNTvsCTR_Difference_in_Pathwaydistance_higher_Score more different between both groups.pdf")
#dev.off()

rankSimilarity2df(cellchat, type = "functional")

write.csv(distance, file="TNTvsCTR_distance.csv")

distance %>% mutate()

ggplot(data=distance, aes(x=dist, y=name, size = dist, text=name)) +
...
#pdf("TNTvsCTR_Informationflow_of_each_pathways_comparison.pdf")
gg <- grid.arrange(gg1 + gg2, nrow = 1)
ggsave(gg, device = "pdf", filename = "TNTvsCTR_Informationflow_of_each_pathways_comparison.pdf")

#dev.off()


#rankNet2df(cellchat, mode = "comparison", stacked = F, do.stat = TRUE, namedf = rank_table2)
rank_table <- rankNet(cellchat, mode = "comparison", stacked = F, do.stat = TRUE, return.data = TRUE)[["signaling.contribution"]] 
rank_table <- rank_table %>% group_by(group) %>% mutate( diff = filter(rank_table,group == "TNT")$contribution.scaled  - filter(rank_table, group == "CTR")$contribution.scaled)

rank_table <-rank_table %>% mutate(namecolor = ifelse(rank_table$pvalues<0.05 && rank_table$group =="TNT", "TNT",ifelse(rank_table$pvalues<0.05 && rank_table$group =="CTR","CTR","NOT")))

write.csv(rank_table, file= "TNTvsCTR_rank_table_org.csv")

rank_table <- read.csv("rank_table_removed_noDifferentToCTRones.csv")
rank_table <- rank_table[order(-rank_table$diff),]

require(scales)
numColors <- length(levels(factor(rank_table$RegvsCTR))) # How many colors you need
...
names(myPalette) <- levels(factor(rank_table$RegvsCTR)) # Give every color an appropriate name



gg1 <- ggplot(data=rank_table, aes(x=diff, y=fct_reorder(name,diff), size = contribution.scaled, text=name, color=group)) +
  geom_point(alpha=0.5) +
  scale_size(range = c(1.4, 18), name="Activity") +
  scale_color_viridis(discrete=T,option = "C", direction = -1) + #, guide=FALSE) +
  theme(legend.position="right") +
  #theme(axis.text.y=element_text(colour=rank_table$Bioprocess)) +
  #theme_ipsum() +
  guides(color = guide_legend(override.aes = list(size = 10))) +
  xlim(-0.1, 0.1) + # Sets limits of y-axis
  #xlim(0, 1) + # Sets limits of x-axis
labs(title="Comparison of activated pathways between both HCM mutants",
      x ="<- activated in MyHC          activated in TnT ->   ", y = "pathways")+ 
  ...
  #theme(axis.title.x = element_text(family, face, colour, size)) + theme(axis.title.y = element_text(family, face, colour, size))

ggsave(gg1, device = "pdf", filename = "TNTvsCTR_Comparison_BubblePlot_pdf", width = 14, height = 10, units = 'in', dpi = 300 )
...
show_col(viridis_pal(option = "C")(20))
  ...
  library(plotly)
  plot_ly(rank_table, x=rank_table$diff, y=rank_table$name, z=rank_table$contribution.scaled, type='scatter3d', mode='markers', 
          size=rank_table$contribution.scaled*100, color=rank_table$group, colorscale = 'Viridis')

  sapply(rank_table,class)
save.image("Cellchat_woweights_TNTvsCTR.Rdata")
...
pdf("TNTvsCTR_Outgoing_Signalpattern_Comparison_Heatmap.pdf")#, paper = "a3")
draw(ht1 + ht2, ht_gap = unit(0.2, "cm"))
#gg1 <- grid.arrange(ht1+ht2)
#ggsave(gg1, device = "pdf", filename = "TNTvsCTR_Outgoing_Signalpattern_Comparison_Heatmap.pdf")
...
pdf("TNTvsCTR_Incoming_Signalpattern_Comparison_Heatmap.pdf")
...
pdf("TNTvsCTR_Overall_Signalpattern_Comparison_Heatmap.pdf")
draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))
dev.off()
...
{
pdf(paste0("TNTvsCTR_ligand-receptor_pairs_Cluster_",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
netVisual_bubble(cellchat, sources.use = sourcecluster, targets.use = targetcluster,  comparison = c(1, 2), angle.x = 45)
dev.off()
#> Comparing communications on a merged object

gg1 <- netVisual_bubble(cellchat, sources.use = sourcecluster, targets.use = targetcluster,  comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in CTR", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
gg2 <- netVisual_bubble(cellchat, sources.use = sourcecluster, targets.use = targetcluster,  comparison = c(1, 2), max.dataset = 1, title.name = "Decreased signaling in CTR", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
pdf(paste0("TNTvsCTR_Increased_Decreased_Signaling_Comparison_by_communication probabities",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
gg1 + gg2
dev.off()
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "CTR",ligand.logFC = 0.2, receptor.logFC = NULL)
# extract the ligand-receptor pairs with upregulated ligands and upregulated recetptors in NL, i.e.,downregulated in LS
net.down <- subsetCommunication(cellchat, net = net, datasets = "TNT",ligand.logFC = -0.1, receptor.logFC = -0.1)
...
#> Comparing communications on a merged object
pdf(paste0("TNTvsCTR_Up-Downregulated_Signaling_Comparison_by_differentialGeneExpression",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
gg1 + gg2
dev.off()
...
# Chord diagram
par(mfrow = c(1,2), xpd=TRUE)
pdf(paste0("TNTvsCTR_Up-Downregulated_Signaling_Comparison_by_differentialGeneExpression_Chordplot",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
...
pdf(paste0("TNTvsCTR_Up-Interactionpathways",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
#
for (i in 1:length(object.list)) {
...
pathways <- intersect(object.list[["TNT"]]@netP[["pathways"]],object.list[["CTR"]]@netP[["pathways"]])
...
pdf(paste0("TNT_vs_CTR_Circleplot_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
#> 
#> Do heatmap based on a single object
pdf(paste0("TNT_vs_CTR_Heatmapt_",x,"_Pathway.pdf"), paper = "a4r")
...
# Chord diagram
pdf(paste0("TNT_vs_CTR_Chordplot_",x,"_Pathway.pdf"), paper = "a4r")
...
pdf(paste0("TNT_vs_CTR_Chordplot_grouped_Celltypes_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
cellchat@meta$datasets = factor(cellchat@meta$datasets, levels = c("TNT", "CTR")) # set factor level
...
ggsave(ggq, device = "pdf", filename = paste0("TNTvsCTR_Genexpression_VlnPlot_Singnalingpathway",x,".pdf"))
}

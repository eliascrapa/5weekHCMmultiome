...
#setwd("~//Users/eliascrapa/OneDrive - UCSF/DATA/SingleCellSeq/Analysis/All/CellChatFiles/!forwOldAllfinalanalysis")
...
cellchat_TNT <- readRDS("cellchat_TNT.rds")
...
pathways <- c("NRXN", "EDA","ANGPTL","CCL", "CD46", "BMP10","ITGAL-ITGB2","NECTIN","CD226","WNT","THY1","SELPLG","CD23","MPZ")
pathways <- c("NGL","CD39","COMPLEMENT","GAS","AGRN","VISFATIN","JAM","FGF","PDGF")
pathways <- c("NRXN", "HSPG","NGL","CD39", "CDH", "BMP10","ITGAL-ITGB2","AGRN","EPHB","EPHA","GALLECTIN","ANGPTL","VWF","EDA","THY1","WNT","ESAM","PERIOSTIN","SEMA7","PTPRM","ANGPT","VCAM")
pathways <- c("NRXN")
pathways <- c("GAS","AGRN")
...
dev.off()
}, error=function(e){cat("ERROR : TnT ",pathways.show, "\n")})
# Circle plot
tryCatch({
png(paste0("TNT_",k,"_signaling__clusters_Circle_Plot.png"), width = 1440, height = 960, units = "px")
par(mfrow=c(1,1))
...
# Circle plot
tryCatch({
png(paste0("MHC_",k,"_signaling__clusters_Circle_Plot.png"), width = 1440, height = 960, units = "px")
par(mfrow=c(1,1))
netVisual_aggregate(cellchat_MHC, signaling = pathways.show, layout = "circle", vertex.label.cex = 3, show.legend = T)
dev.off()
}, error=function(e){cat("ERROR : MyHC ",pathways.show, "\n")})
...
pathways.show <- list()
pathways.show <- c("TGFb","PERIOSTIN","NRG","EPHB")
